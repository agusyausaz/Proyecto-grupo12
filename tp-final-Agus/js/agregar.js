...
function confirmarAgregar(){
    ...
    actualizarPantalla(); // Actualizar la lista de películas disponibles
    mostrarEnCarrito(pelicula); // Mostrar la película agregada en el carrito
}
...
function mostrarEnCarrito(pelicula) {
    const modalCarrito = document.getElementById('peliculasSeleccionadas');
    const baseUrl = 'https://image.tmdb.org/t/p/w500';

    const html = `
        <div class="pelicula-seleccionada">
            <img src="${baseUrl + pelicula.poster_path}" alt="${pelicula.title}">
    ...
    modalCarrito.insertAdjacentHTML('beforeend', html);
}